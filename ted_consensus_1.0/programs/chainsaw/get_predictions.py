...
def main(args):
    ...
    if input_method == 'structure_directory':
        ...
        for idx, fname in enumerate(os.listdir(structure_dir)):
            ...
            # IS: local modification - skip missing files on CS HPC
            pdb_path = os.path.join(structure_dir, fname)
            if not Path(pdb_path).exists():
                LOG.warning(f"Failed to find model file {pdb_path} (skipping)")
    ...
    if skipped_file_count > 0:
        LOG.error(f"Skipped {skipped_file_count} files")
        LOG.info("DONE")
        sys.exit(1)
...
def parse_args():
    ...
    parser.add_argument('--remove_disordered_domain_threshold', type=float, default=0.0,
                        help='if the domain is less than this proportion secondary structure, it will be removed')
    parser.add_argument('--min_domain_length', type=int, default=10,
                        help='if the domain has fewer residues than this it will be removed')
    parser.add_argument('--min_ss_components', type=int, default=1,
